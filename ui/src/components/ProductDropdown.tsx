...
export default function ProductDropdown({ products, activeProductId, onSelect, onNewProduct, onOverview, showingOverview }: Props) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        {showingOverview ? (
          <span className="text-adj-text-faint text-sm">⊞</span>
        ) : active ? (
          <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: active.color }} />
        ) : null}
        <span>{showingOverview ? 'Overview' : (active?.name ?? 'Select product')}</span>
      ...
      {open && (
        <div id="product-listbox" role="listbox" aria-labelledby="products-label" className="absolute top-full left-0 mt-1 w-52 bg-adj-surface border border-adj-border rounded-lg shadow-xl z-50 overflow-hidden">
          {onOverview && (
            <>
              <button
                onClick={() => { onOverview(); setOpen(false) }}
                className="w-full flex items-center gap-3 px-3 py-2 text-sm text-left hover:bg-adj-elevated transition-colors"
              >
                <span className={`text-sm ${showingOverview ? 'text-adj-accent' : 'text-adj-text-faint'}`}>⊞</span>
                <span className={showingOverview ? 'text-adj-text-primary font-medium' : 'text-adj-text-secondary'}>
                  Overview
                </span>
                {showingOverview && <span className="ml-auto text-adj-accent text-xs">✓</span>}
              </button>
